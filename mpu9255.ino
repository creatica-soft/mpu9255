...
#define FIFO_EN_SLV_2 4 //write EXT_SENS_DATA registers associated to SLV_2 (as determined by I2C_SLV0_CTRL, I2C_SLV1_CTRL, and I2C_SL20_CTRL) to the FIFO at the sample rate;
#define FIFO_EN_SLV_1 2 //write EXT_SENS_DATA registers associated to SLV_1 (as determined by I2C_SLV0_CTRL and I2C_SLV1_CTRL) to the FIFO at the sample rate;
...
uint8_t magnetometerSelfTest() {
  ...
  for (uint8_t i = 0; i < 3; i++) senseAdj[i] = ((int16_t)xyz[i] - 128)/256.0 + 1;
  sprintf(s, String(F("senseAdj x = %f, y = %f, z = %f")).c_str(), senseAdj[0], senseAdj[1], senseAdj[2]);
  Serial.println(s);
  ...
  setAkmPowerMode(AKM_CNTL_MODE_POWER_DOWN);
  data[0] = ((((int16_t)xyz[1]) << 8) | xyz[0]) * senseAdj[0];
  data[1] = ((((int16_t)xyz[3]) << 8) | xyz[2]) * senseAdj[1];
  data[2] = ((((int16_t)xyz[5]) << 8) | xyz[4]) * senseAdj[2];
...
uint8_t readMagnetometer(XYZ * m) {
  ...
  m->x = (((int16_t)xyz[1]) << 8) | xyz[0];
  m->y = (((int16_t)xyz[3]) << 8) | xyz[2];
  m->z = (((int16_t)xyz[5]) << 8) | xyz[4];
  return 0;
}

void confMpu(uint8_t conf) {
  uint8_t err;
...
void setup() {
  ...
  i2cBypass(true);
  delay(100);
  Serial.println(String(F("AK8963 (compass) Id: ")) + getChipId(AKM_I2C_ADDR, WIA));
  ...
  delay(100);
  //sampleRateDivider(4); //divides 1kHz internal sampling (FIFO, output data rate ODR) rate: 
  //new rate = 1kHz / (1 + divider) - according to the manual or is it new rate = 1kHz / (2^divider) ??? 
...
void loop() {
  ...
  XYZ g, a, m;
  memset((XYZ *)&g, 0, sizeof(XYZ));
  memset((XYZ *)&a, 0, sizeof(XYZ));
  memset((XYZ *)&m, 0, sizeof(XYZ));
